...
<head>
    ...
    <title>الأنشطة</title>
    <link rel="stylesheet" href="style/events-style.css">
...
<body>
    <nav class="navbar">
        ...
        <ul class="links">
            ...
            <li><a href="events.php" style="opacity:1;">الأنشطة</a></li>
    ...
    <div class="event-header-div">
        <p class="event-header"><span class="main-header"><a href="main.php">الرئيسية</a> </span><span class="page-header">> النشاطات</span></p>
    </div>

    <!--creating new event (only visible for leaders)-->     
        <?php
            if($rank == "قائد") {
                echo '<div class="new-event-div">
                        <button type="button" id="new-event-btn">انشاء نشاط جديد...</button>
                    </div>';
                }
        ?>
    </div>
    ...
    <div class="container" id="events"> 
        <?php 
            $sql1 = "SELECT * FROM event WHERE start_date >= CURDATE() AND deleted = 0 AND branch_id = ? ORDER BY start_date ASC";
            ...
            if(!mysqli_stmt_prepare($stmt1,$sql1)) {
                ...
                $rowNoEvents = mysqli_num_rows($result);

                //get the events records (rows) and display them
                while($row = mysqli_fetch_assoc($result)) {

                    //check if the user is registered in this event or not
                    $sql2 = "SELECT * FROM event_registration WHERE user_id = ". $_SESSION["userId"] . " AND event_id =" . $row["event_id"];
                    //exexcute the query
                    $result2 = mysqli_query($conn,$sql2);

                    $rowNoUserRig = mysqli_num_rows($result2);

                    echo '<div class="event-container">';
                    if($rank == "قائد")
                        echo '<div class="event-menu-btn">
                    ...
                    echo '<div class="event-menu">
                             <ul>
                                <li id="delete-option" class="delete-option" ><a href="#" data-event-id="'. $row["event_id"] .'">حذف</a></li>
                                <li id="edit-option" class="edit-option"><a href="#" data-event-title ="'. $row["title"] .'" data-event-startdate="'. $row["start_date"] .'" data-event-description="'. $row["description"] .'" data-event-image="'. $row["image_path"] .'" data-event-id="'. $row["event_id"] .'">تعديل</a></li>
                             </ul>
                         </div>';
                    ...
                    if($rank == "عضو") {
                        if($rowNoUserRig > 0)
                            echo '<br>
                                ...
                                <button type="button" id="cancel-register-btn" class="register-cancel-btn" data-event-id="'.$row["event_id"] . '">إالغاء التسجيل</button>
                                </div>';
                        else
                            echo '<br><div style="margin-top:10px;"><button type="button" id="event-register-btn"  data-event-id="'.$row["event_id"] . '">سـجـل الأن</button></div>';
                    }
                    else {
                            echo '<br><div style="margin-top:10px;" data-event-id="'.$row["event_id"].'"><button type="button" id="event-show-registers" class="event-show-registers">عرض المسجلين</button></div>';
                    }
                ...
                if($rowNoEvents == 0) {
                    echo '<h3 class="no-events-text">لا يوجد أنشطة حاليًا</h3>';
                }
            }
    ...
    <!-- cancel/confirm cancel registration -->
     <div class="pop-up" id="pop-up-cancel-reg">
        <div class="pop-up-inner" id="cancel-register-inner">
            ...
            <p>هل انت متأكد من أنك تريد إالغاء التسجيل بهذا النشاط</p>
    ...
    <!-- cancel/confrim confirm registration -->
     <div class="pop-up" id="pop-up-reg">
            <div class="pop-up-inner" id="register-inner">
                ...
                <p>تم تسجيلك بهذا النشاط بنجاح</p>
    ...
    <div class="pop-up" id="pop-up-deletion">
        <div class="pop-up-inner" id="delete-inner">
            ...
            <p>هل أنت متأكد أنك تريد حذف هذا النشاط!</p>
    ...
    <div class="pop-up-create" id="pop-up-create">
        <div class="wrapper" id="wrapper">
            ...
            <form action="includes/event.inc.php" method="post" id="myForm" enctype="multipart/form-data">
                ...
                <div class="buttons">
                    <button type="submit" class="create-btn" id="create-btn" name="create-event">إنشاء</button>
    ...
    <div id="pop-up-edit" class="pop-up-edit">
            <div class="wrapper" id="wrapper2">
                ...
                <form action="includes/event.edit.inc.php" method="post" id="myForm2" enctype="multipart/form-data">
                    <!--dummy feild to pass the event id to the back end-->
                    <input type="text" style="display:none" hidden name="event-id" id="event-id">
                    ...
                    <div class="buttons">
                        <button type="submit" class="edit-btn" id="edit-btn" name="edit-event">تعديل</button>
