...
<body>
    ...
    <div class="memebers-header-div">
        <p class="memebers-header"><span class="main-header"><a href="main.php">الرئيسية</a> </span><span class="page-header">> <a href="./events.php">النشاطات</a></span><span class="page2-header"> > الاعضاء المسجلين</span></p>
    </div>
    <br>
    <hr style="width:60%">
    ...
    <?php
        ...
        $sql2 = 'SELECT * FROM event WHERE event_id=' . $_GET['eventId'];
        $result2 = mysqli_query($conn,$sql2);

        $row2 = mysqli_fetch_assoc($result2);
        if($row2 > 0) {
            echo '<div class="title-header haeder">
                    <h3>اسم النشاط: ' . $row2['title'] . '</h3>
                    <h3>رقم النشاط: ' . $row2['event_id'] . '</h3>
                  </div>';
        }   

        //select the memebers regietered in the event
        $sql3 = 'SELECT fname, midname, lastname,email FROM event_registration as ev INNER JOIN user as us on ev.user_id = us.id WHERE event_id = ' . $_GET['eventId'] . ' ORDER BY created_at';
