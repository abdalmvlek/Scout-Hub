<?php
    ...
    if(isset($_POST["create-volunteerwork"])) {
        $db_server = "localhost";
        $db_user = "root";
        $db_pass = "";
        $db_name = "scouthub";
        ...
        $sql = "INSERT INTO volunteerwork (title,description,start_date,created_by,branch_id,image_path)
                VALUES (?,?,?,?,?,?)";
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)) {
            ...
            header("Location: ../volunteerWork.php?msg=success");
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
    }
    else {
        header("Location: ../volunteer.php?error=AccsessDenied");
        exit;
    }
?>